...
from vision.datasets.arg_dataset import ARGDataset
...
parser.add_argument("--dataset_type", default="arg", type=str,
                    help='Specify dataset type. Currently support voc and open_images.')

parser.add_argument('--datasets', nargs='+', default=['/media/arg_ws3/5E703E3A703E18EB/data/argbot'],\
                                                        help='Dataset directory path')
parser.add_argument('--validation_dataset', default='/media/arg_ws3/5E703E3A703E18EB/data/argbot', help='Dataset directory path')
...
parser.add_argument('--base_net', default="models/mobilenet_v1_with_relu_69_5.pth",
...
parser.add_argument('--save_model_folder', default='models/argbot',
                    help='Directory for saving checkpoint models')
...
def train(loader, net, criterion, optimizer, device, debug_steps=100, epoch=-1):
    ...
    for i, data in enumerate(loader):
        ...
        if i and i % debug_steps == 0:
            ...
            logging.info(
                "Epoch: {}, Step: {}/{}, ".format(epoch, i, len(loader)) +
...
if __name__ == '__main__':
    ...
    for dataset_path in args.datasets:
        if args.dataset_type == 'voc':
            ...
            label_file = os.path.join(args.save_model_folder, "voc-model-labels.txt")
            store_labels(label_file, dataset.class_names)
            num_classes = len(dataset.class_names)
        elif args.dataset_type == 'arg':
            dataset = ARGDataset(dataset_path, transform=train_transform,
                                 target_transform=target_transform)
            label_file = os.path.join(args.save_model_folder, "arg-model-labels.txt")
            store_labels(label_file, dataset.class_names)
            num_classes = len(dataset.class_names)
            ...
            label_file = os.path.join(args.save_model_folder, "open-images-model-labels.txt")
            store_labels(label_file, dataset.class_names)
    ...
    if args.dataset_type == "arg":
        val_dataset = ARGDataset(args.validation_dataset, transform=test_transform,
                                 target_transform=target_transform, is_test=True)
    elif args.dataset_type == 'open_images':
